...
To ensure reproducibility a ndorganized storage, we programmatically create directories for:
...
```
:::
We also inspect the dataset shape. We see that the data has *32,561* rows and *15* columns.
...
|                 | Prof-specialty        | Professional occupations (e.g. scientist, lawyer)                |
...
| **sex**         | Female                | Female                                                           |
|                 | Male                  | Male                                                             |
| **native-country** | Various (e.g. United-States, Cambodia, England, etc.) | Country of origin          |
...
```{python}
#| output: false
adult_df['education_level'] = adult_df['education_level'].fillna('unknown')
```
